...
const IphoneSketches = () => {
  ...
  const displaySketches = sketches?.length ? sketches.map(sketch => ({
    id: sketch.id,
  ...
  return <div className="min-h-screen bg-background">
      ...
      <main className={`w-full md:w-[calc(100vw-120px)] min-h-screen transition-transform duration-300 ${isMenuOpen ? 'translate-x-full md:translate-x-[300px]' : 'translate-x-0'} ${'md:ml-[120px]'}`}>
        <div className="max-w-7xl mx-auto px-4 py-8">
          ...
          <p className="text-muted-foreground mb-12 max-w-2xl">We are artists, friends, and just generally happy people who want to lift eachother up.

Get to know us.</p>

          {isLoading && <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
              {[1, 2, 3, 4, 5, 6].map((_, idx) => <div key={idx} className="relative w-[280px] h-[580px] rounded-[45px] bg-card/50 animate-pulse">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-12">
            {filteredSketches.map(sketch => <div key={sketch.id} className="group relative mx-auto">
                {/* iPhone Frame */}
                <div className="relative w-[280px] h-[580px] rounded-[45px] bg-card shadow-xl p-3 overflow-hidden">
                  ...
                  <div className="relative w-full h-full rounded-[35px] overflow-hidden bg-background">
                    <img src={sketch.src} alt={sketch.title} className="h-full w-full object-cover transition-transform duration-300 group-hover:scale-110" />
